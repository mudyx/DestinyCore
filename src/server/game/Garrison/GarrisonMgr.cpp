...
GarrisonClassHallPlotGOInfo const* GarrisonMgr::GetPlotClassHallGOInfo(uint32 garrPlotInstanceID) const
{
    auto itr = _classHallPlotGOInfo.find(garrPlotInstanceID);
    if (itr != _classHallPlotGOInfo.end())
        return &itr->second;

    return nullptr;
}

std::vector<GarrisonClassHallPlotGOInfo> GarrisonMgr::GetPlotClassHallByCreatureId(uint32 creatureID) const
{
    std::vector<GarrisonClassHallPlotGOInfo> list;
    for (const std::pair<uint32, GarrisonClassHallPlotGOInfo> itr : _classHallPlotGOInfo)
        if (itr.second.CreatureId == creatureID)
...
void GarrisonMgr::LoadPlotClassHallGOInfo()
{
    //                                                       0                1            2        3     4     5     6
    QueryResult result = WorldDatabase.Query("SELECT garrPlotInstanceId, creatureId, gameobjectId, posX, posY, posZ, posO, "
        //       7          8                  9         
        "workDisplayId, completeDisplayId, animKitId FROM garrison_plot_class_hall");
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 garrison plot class hall. DB table `garrison_plot_class_hall` is empty.");
        return;
    }
    ...
    do
    {
        ...
        uint32 gameObjectId = fields[2].GetUInt32();
        uint32 workDisplayId = fields[7].GetUInt32();
        uint32 completeDisplayId = fields[8].GetUInt32();
        uint16 animKitId = fields[9].GetUInt16();

        GameObjectTemplate const* goTemplate = sObjectMgr->GetGameObjectTemplate(gameObjectId);
        if (!goTemplate)
        {
            TC_LOG_ERROR("sql.sql", "Non-existing gameobject_template entry %u was referenced in `garrison_plot_class_hall`.`gameobjectId` for garrPlotInstanceId %u.",
                gameObjectId, garrPlotInstanceId);
            continue;
        }

        if (goTemplate->type != GAMEOBJECT_TYPE_GARRISON_SHIPMENT)
        {
            TC_LOG_ERROR("sql.sql", "Invalid gameobject type %u (entry %u) was referenced in `garrison_plot_class_hall`.`gameobjectId` for garrPlotInstanceId %u.",
                goTemplate->type, gameObjectId, garrPlotInstanceId);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Non-existing AnimKit.dbc entry %u was referenced in `garrison_plot_class_hall`.`animKitId` for garrPlotInstanceId %u.",
                animKitId, garrPlotInstanceId);
            continue;
        }


        GarrisonClassHallPlotGOInfo& info = _classHallPlotGOInfo[garrPlotInstanceId];
        info.PlotId = garrPlotInstanceId;
        info.CreatureId = creatureId;
        info.GameObjectId = gameObjectId;
        info.Pos.Relocate(fields[3].GetFloat(), fields[4].GetFloat(), fields[5].GetFloat(), fields[6].GetFloat());
        info.AnimKitId = animKitId;
        info.workDisplayId = workDisplayId;
        info.completeDisplayId = completeDisplayId;
    } while (result->NextRow());

    TC_LOG_INFO("server.loading", ">> Loaded %u garrison plot class hall entries in %u.", uint32(_classHallPlotGOInfo.size()), GetMSTimeDiffToNow(msTime));
}
