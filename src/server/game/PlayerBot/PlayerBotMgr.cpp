...
uint8 PlayerBotMgr::RandomFacialHair(uint8 race, uint8 gender, uint8 hairColor, uint8 prof)
{
    ...
    TC_LOG_DEBUG("server.loading", "Random player custom FacialHair timeout!");
    return 0;
}
...
void PlayerBotMgr::CreateOncePlayerBot()
{
        ...
        it++)
    {
        ...
        if (pInfo->needCreateBots.size() > 0)
        {
            ...
            if (pSession)
            {
                // PLAYERBOT BEGIN: create character via modern CreateCharacter API
                // Parse the stored packet (legacy simple format) back into fields
                WorldPacket data(packet);
                std::string name;
                uint8 race, cls, gender, skinColor, faceId, hairId, hairColor, facialHair, outfit;
                data >> name;
                data >> race;
                data >> cls;
                data >> gender;
                ...
                using namespace WorldPackets::Character;
                // Prepare CreateCharacter packet structure
                // původní konstrukce bez opkódu ponechána pro referenci
                // CreateCharacter createPkt(std::move(WorldPacket()));
                WorldPacket dummy(CMSG_CREATE_CHARACTER);
                CreateCharacter createPkt(std::move(dummy));
                createPkt.CreateInfo = std::make_shared<CharacterCreateInfo>();
                auto& info = *createPkt.CreateInfo;
                info.Name = name;
                info.Race = race;
                info.Class = cls;
                info.Sex = gender;
                info.Skin = skinColor;
                info.Face = faceId;
                info.HairStyle = hairId;
                info.HairColor = hairColor;
                info.FacialHairStyle = facialHair;
                info.OutfitId = outfit;

                // Send through session handler
                pSession->HandleCharCreateOpcode(createPkt);
                // PLAYERBOT END
            }
...
void PlayerBotMgr::SupplementAccount()
{
    ...
    TC_LOG_INFO("server.loading", "[PlayerBot] SupplementAccount: needAccount=%u, existing=%zu", needAccount, m_idPlayerBotBase.size());
    if (needAccount <= 0)
        return;
    ...
    for (uint32 i = 0; i < needAccount; ++i)
    {
        ...
        TC_LOG_INFO("server.loading", "[PlayerBot] Attempt create account %s", userName.c_str());
...
void PlayerBotMgr::Update()
{
    ...
    // Po úspěšné tvorbě nebo ověření účtů doplň postavy, pokud alespoň jeden bot account existuje
    if (!m_idPlayerBotBase.empty())
    {
        TC_LOG_INFO("server.loading", "[PlayerBot] Calling SupplementPlayerBot to generate characters");
        SupplementPlayerBot();
    }
}
